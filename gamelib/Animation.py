...
class Animation(object):
    ...
    An BoardItem can have an animation and all of them that are available to the Game object can be animated through Game.animate_items(lvl_number).
    ...
    :param auto_replay: controls the auto replay of the animation, if false once the animation is played it stays on the last frame of the animation.
    ...
    :param refresh_screen: The callback function that controls the redrawing of the screen. This function reference should come from the main game.
    ...
    Example ::
        ...
        # By default BoardItem does not have any animation, we have to explicitely create one
        item.animation = Animation(display_time=0.1, animated_object=item, refresh_screen=redraw_screen)
    """
    def __init__(self,display_time=0.05,auto_replay=True,frames=None,animated_object=None, refresh_screen=None):
            self.state = RUNNING
    ...
    def next_frame(self):
        ...
        If animated_object is not a sub class of :class:`~gamelib.BoardItem.BoardItem` an exception is raised.
        ...
        if not isinstance(self.animated_object, BoardItem):
            raise HacInvalidTypeException('The animated_object needs to be a sub class of BoardItem.')
    ...
    def play_all(self):
        ...
        if self.refresh_screen is None or not callable(self.refresh_screen):
            raise HacInvalidTypeException('The refresh_screen parameter needs to be a callback function reference.')
        if not isinstance(self.animated_object, BoardItem):
            raise HacInvalidTypeException('The animated_object needs to be a sub class of BoardItem.')
