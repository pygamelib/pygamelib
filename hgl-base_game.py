...
if sprite_mode == "sprite":
    # sprite_player = graphics.Models.SHEEP
    sprite_portal = graphics.Models.CYCLONE
    sprite_treasure = graphics.Models.GEM_STONE
    sprite_treasure2 = graphics.Models.MONEY_BAG
    sprite_tree = graphics.Models.EVERGREEN_TREE
    sprite_wall = graphics.Models.BRICK
    sprite_npc = graphics.Models.SKULL
    sprite_npc2 = graphics.Models.PILE_OF_POO
    sprite_heart = graphics.Models.SPARKLING_HEART
    sprite_heart_minor = graphics.Models.BLUE_HEART
else:
    # sprite_player = Utils.RED_BLUE_SQUARE
    sprite_portal = graphics.CYAN_SQUARE
    sprite_treasure = graphics.YELLOW_RECT + graphics.RED_RECT
    sprite_treasure2 = base.Text.yellow_bright("$$")
    sprite_tree = graphics.GREEN_SQUARE
    sprite_wall = graphics.WHITE_SQUARE
    sprite_npc = base.Text.magenta_bright("oO")
    sprite_npc2 = base.Text.magenta_bright("'&")
    sprite_heart = base.Text.red_bright("<3")
    sprite_heart_minor = base.Text.blue_bright("<3")
...
bg2_actuator = actuators.PathActuator(path=[cst.RIGHT for k in range(0, 30, 1)])
for k in range(0, 30, 1):
    bg2_actuator.path.append(cst.LEFT)
game.add_npc(
    1,
    board_items.NPC(model=sprite_npc, name="Bad guy 2", actuator=bg2_actuator, step=1,),
    7,
    9,
)
...
for k in range(0, 20, 1):
    game.add_npc(2, board_items.NPC(model=sprite_npc2, name=f"poopy_{k}", step=1))


# Now let's add a nice NPC, one we can talk to!
nice_npc = board_items.NPC(
    name="Unipici", model=graphics.Models.UNICORN, step=0
...
while key != "q":
    ...
    # Here is the interaction menu with Unipici
        ...
        if key == "4":
            ...
            print(nice_npc.model + base.Text.magenta_bright(riddle["q"]))
