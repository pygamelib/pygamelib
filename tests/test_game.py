...
class TestBase(unittest.TestCase):
    def test_game_create(self):
        ...
        engine.Game(user_update=user_update_placeholder)
    ...
    def test_config(self):
        ...
        self.assertIsNone(
            g.save_config("high_scores", "test-pygamelib.engine.Game.json")
        )
        with self.assertRaises(base.PglInvalidTypeException):
            g.save_config(None, "test-pygamelib.engine.Game.json")
        with self.assertRaises(base.PglInvalidTypeException):
            g.save_config("high_scores", None)
        with self.assertRaises(base.PglException) as e:
            g.save_config("Unknown", "test-pygamelib.engine.Game.json")
            self.assertEqual(e.error, "unknown section")
        # Don't do that...
        g._configuration = None
        g._configuration_internals = None
        g.load_config("test-pygamelib.engine.Game.json", "new_high_scores")
