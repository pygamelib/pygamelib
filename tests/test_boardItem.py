...
class TestBoard(unittest.TestCase):
    ...
    def test_boarditemvoid(self):
        bi = pgl_board_items.BoardItemVoid()
    ...
    def test_boarditemcomplexcomponent(self):
        bi = pgl_board_items.BoardItemComplexComponent(parent=None)
        self.assertFalse(bi.restorable())
        self.assertFalse(bi.overlappable())
        self.assertFalse(bi.can_move())
        self.assertFalse(bi.pickable())
        bic = pgl_board_items.ComplexPlayer()
        bi = pgl_board_items.BoardItemComplexComponent(parent=bic)
        bic = pgl_board_items.Tile()
    ...
    def test_boardcomplexitem(self):
        ...
        with self.assertRaises(pgl_board_items.base.PglOutOfItemBoundException):
            bic.item(5, 6)

    def test_movable(self):
        bi = pgl_board_items.Movable(step=2)
        bi = pgl_board_items.Movable(step_vertical=2, step_horizontal=1)
    ...
    def test_projectile(self):
        with self.assertRaises(pgl_board_items.base.PglException) as e:
            pgl_board_items.Projectile(range=6, step=4)
            self.assertEqual(e.error, "incorrect_range_step")
        p = pgl_board_items.Projectile()
